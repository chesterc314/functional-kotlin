...
object ThirdPartyFunctionalLibraryExTry {
    ...
    private fun arrowTryUsage(): String {
        ...
        val filterCondition = if (failedCondition.isFailure()) {
            "You can use `filter` to cause a failure when the predicate is not satisfied, this will return a `Failure`"
        } else {
            ""
        }
        ...
        val default: List<String> = tryGetLotteryNumbers.getOrDefault { emptyList() }
        ...
        return when (tryGetLotteryNumbers) {
            ...
            is Failure -> "* The `Try` object is used to handle exceptions in a more graceful manner," +
                    " when the execution is successful it returns `Success` (you can check the status of the Try with`isSuccess`)" +
                    "or an exception occurs it returns `$status` (you can check the status of the Try `isFailure`)\n" +
                    "* $filterCondition\n" +
                    "* The `getOrDefault` function allows you to set a default when a failure occurs \n" +
